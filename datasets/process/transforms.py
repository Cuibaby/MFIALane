...
class ToTensor(object):
    ...
    def __init__(self, keys=['img', 'label'], cfg=None):
        self.keys = keys
...
class Resize(object):
    ...
    def __call__(self, sample):
        ...
        if 'label' in sample:
            sample['label'] = cv2.resize(sample['label'], self.size,
...
class RandomRotation(object):
    ...
    def _rotate_mask(self, sample, map_matrix):
        if 'label' not in sample:
            return
        h, w = sample['label'].shape[0:2]
        sample['label'] = cv2.warpAffine(
            sample['label'], map_matrix, (w, h), flags=cv2.INTER_NEAREST, borderMode=cv2.BORDER_CONSTANT, borderValue=self.padding)
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, sample):
        ...
        if v < 0.5:
            ...
            if 'label' in sample: sample['label'] = np.fliplr(sample['label'])
        return sample
