...
class ResNetWrapperV(nn.Module):
    ...
    def __init__(self, 
                ...
                cfg=None):
        super(ResNetWrapperV, self).__init__()
        self.cfg = cfg
    ...
    def forward(self, x):
        x = self.model(x)
        if self.out: #[layer1,layer2,layer3,layer4]
            x[-1] = self.out(x[-1])
        return x  #list  [layer1,layer2,layer3,layer4]
...
class ResNetV(nn.Module):
    ...
    def __init__(self, block, layers, zero_init_residual=False,
        ...
        self.conv3 = nn.Conv2d(32, self.inplanes, kernel_size=3, stride=1, padding=1,
                               bias=False)
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    model = ResNetV(block, layers, **kwargs)
    if pretrained:
