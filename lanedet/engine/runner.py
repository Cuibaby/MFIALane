...
from lanedet.utils.net_utils import save_model, load_network, load_network_specified
...
class Runner(object):
   ...
   #     input_img = (3,self.cfg.img_height,self.cfg.img_width)
   #     flops, params = get_model_complexity_info(self.net.eval(), input_img)
   #     print(f'{split_line}\nInput shape: {input_shape}\n'
   #       f'Flops: {flops}\nParams: {params}\n{split_line}')

        parameters = sum(p.numel() for p in self.net.parameters() if p.requires_grad)
        self.recorder.logger.infont('the model parameter is : '+ str(parameters / 1e6)+'M')
      #  self.net.to(torch.device('cuda'))range(self.cfg.gpus)
      ...
      #          self.net, device_ids = range(self.cfg.gpus)).cuda()
        ...
        self.warmup_scheduler = warmup.LinearWarmup(
            self.optimizer, warmup_period=500)
    ...
    def train(self):
        ...
        for epoch in range(self.cfg.epochs):
            ...
            if (epoch + 1) % self.cfg.eval_ep == 0 or epoch == self.cfg.epochs - 1 or self.recorder.step >= self.cfg.total_iter:
                self.validate()
