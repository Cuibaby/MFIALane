...
list_files = {
'trainval':'seg_label/list/train_val_gt.txt',
'val': 'seg_label/list/test_gt.txt',
...
class TuSimple(BaseDataset):
    def __init__(self, data_root, split, processes=None, cfg=None):
        ...
        self.anno_files = list_files[split] # SPLIT_FILES[split] 
    ...
    def load_annotations(self):
        ...
        self.data_infos = []
        max_lanes = 0
        anno_file = osp.join(self.data_root, self.anno_files)
        with open(anno_file, 'r') as lines:
             for line in lines:
                line_split = line.strip().split(" ")
                exist_list = np.array([int(line_split[2]), int(line_split[3]),
                              int(line_split[4]), int(line_split[5]),
                              int(line_split[6]), int(line_split[7])
                              ])
                self.data_infos.append({
                    'img_path': osp.join(self.data_root, line_split[0][1:]),
                    'img_name': line_split[0][1:],
                    'mask_path': osp.join(self.data_root, line_split[1][1:]),
                    'lane_exist': exist_list,
                })
...
#                gt_lanes = data['lanes']
#                mask_path = data['raw_file'].replace('clips', 'seg_truth')[:-3] + 'png'
